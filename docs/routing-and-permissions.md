...
如果手动配置 hook，推荐使用初始化命令自动写入的 `uvx cc-web-mcp hook-guard` 形式。只有在 `cc-web-mcp` 已经通过 `pipx` 或 PATH 可见的本地安装方式可直接运行时，才使用示例里的 `cc-web-mcp hook-guard` console script；这样可以避免把具体仓库路径或 Python 解释器路径写死在 `settings.json` 里。
